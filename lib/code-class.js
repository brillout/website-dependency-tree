...
function Code(code_info){
    ...
    this.type = code_info.type || get_type(code_info.uri, code_info.includer);

    this.location = new Location({
        type: this.type,
        uri: code_info.uri,
        path: code_info.path,
        includer: code_info.includer
    });

    this.includer = code_info.includer;

    this.source_code = code_info.source_code || fetch(this.location) || null;
    this.hash = this.source_code && compute_sha256(this.source_code) || null;

    this.id = get_id(this);

    this.dependencies = null || code_info.dependencies;
...
function Location(code_info){
    ...
    this.disk = {
        path:
            code_info.path
            || this.uri
                && ! is_url(this.uri, code_info.includer)
                && get_path(code_info.uri, code_info.type, code_info.includer)
            || null
    };

    this.disk.directory =
        this.disk.path
        && require('path').dirname(this.disk.path) || code_info.includer.location.disk.directory ;

    this.internet = {
        url:
            this.uri
                && is_url(this.uri, code_info.includer)
                && get_url(code_info.uri, code_info.includer)
            || null
    };

    // addresses are absolute
    assert( this.disk.path === null || require('path').parse(this.disk.path).root );
    assert( this.internet.url === null || require('url').parse(this.internet.url).host );

    // directory should always exist
    assert( this.internet.url !== null || this.disk.directory );
    ...
    function get_url(uri, includer){
        var info__as_url = require('url').parse(uri);

        assert( info__as_url.host || includer );

        if( info__as_url.host ) {
            return info__as_url.href;
        }
    ...
    function get_path(uri, type, includer){

        var info__as_path = require('path').parse(uri);

        if( ! includer ) {

            // "entry point code"

            if( info__as_path.root ) {
                return uri;
            }
            else {
            ...
            var related_includer =
                !! info__as_path.root ?
                    ...
                    jump_inline_includer(includer) ;

            assert( related_includer && related_includer.location.disk.path );

            return require('path').resolve(
                       require('path').dirname(related_includer.location.disk.path) ,
                       uri );
        ...
        function jump_inline_includer(includer){
            if( includer.location.disk.path === null && includer.location.internet.url === null ){
                return includer.includer;
            }
            return includer;
        }
...
function fetch(location){
    ...
    if( location.internet.url ) {
        log.calc("fetching `"+location.internet.url+"`");
        var body =
            require('sync-request')(
                'GET',
                location.internet.url)
                    .getBody()
                    .toString();
        log.ok();
        return body;
...
function get_type(uri, includer){
    ...
    var suffix = get_suffix(require('url').parse(uri).path);
...
function is_url(uri, includer){
    ...
    assert( includer || info__as_url.host || info__as_path.root );
    ...
    assert( info__as_path.root );

    return false;
}

function get_id(code){
    Object.observe(code.location, set_id);
    Object.observe(code.location.disk, set_id);
    Object.observe(code.location.internet, set_id);
    ...
    function compute(){
        return code.location.internet.url || code.location.disk.path || code.location.uri || code.hash;
    }
}
