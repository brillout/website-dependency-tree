...
function cache(code_pristine){
    ...
    var code_cached = cache_map[code_id];

    if( ! code_cached ) {
        cache_map[code_id] = code_pristine;
        return null;
    }

    code_cached._last_input.last_includer = code_pristine._last_input.last_includer;
    code_cached.last_includer = code_pristine.last_includer;
