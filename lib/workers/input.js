...
function validate_input(code){
    ...
    if( ! input || ! input.last_inclusion || ! input.last_inclusion.uri && ( ! input.last_inclusion.includer || typeof(input.source_code) === 'undefined' ) )
        throw "Code Consructor needs object argument with `.last_inclusion.uri` or `.last_inclusion.includer` and `.source_code` ; `new Code({last_inclusion: {uri: '/bla.html'}})` / `new Code({last_inclusion: {includer: new Code(/*...*/)}, source_code: new Buffer('/* src code */')})";
    if( input.uri )
        throw "uri needs to be defined on `.last_inclusion` property; `new Code({last_inclusion: {uri: '/bla.html' }})`";
    if( input.path || (input.location||{}).path)
