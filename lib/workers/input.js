...
function validate_input(code){
    ...
    if( ! input || ! input.last_inclusion || ! input.last_inclusion.uri && ( ! input.last_inclusion.includer || typeof(input.source_code) === 'undefined' ) )
        throw [
            "CodeInfo Consructor needs object argument with `.last_inclusion.uri` or `.last_inclusion.includer` and `.source_code`;",
            "    `new CodeInfo({last_inclusion: {uri: '/bla.html'}})`",
            "/",
            "    `new CodeInfo({last_inclusion: {includer: new CodeInfo(/*...*/)}, source_code: new Buffer('/* src code */')})",
            "instead input ===",
...
function set_root_includer(code){
    ...
    assert( !! code.root_includer === !! code.last_inclusion.includer );
