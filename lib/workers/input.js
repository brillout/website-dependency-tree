...
function validate_input(code){
    ...
    assert( (code._last_input.location||{}).uri || typeof(code._last_input.source_code) !== 'undefined' && code._last_input.includer );

    assert( ! code._last_input.includer || code._last_input.includer.location.disk.directory || code._last_input.includer.location.internet.url );
}

function set_root_includer(code){
    code.root_includer = code.root_includer || (code.includer||{}).root_includer || code.includer || null;
}
