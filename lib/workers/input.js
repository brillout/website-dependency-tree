...
function assign_input(code){
    ...
    last_inclusion.depender = last_inclusion.depender || null;
...
function validate_input(code){
    ...
    if( input && ( input.uri || (input.location||{}).uri ) )
        throw "uri needs to be defined on `.last_inclusion` property; `new CodeInfo({last_inclusion: {uri: '/bla.html' }})`";
    ...
    if( ! input || ! input.last_inclusion || ! input.last_inclusion.uri && ( ! input.last_inclusion.depender || typeof(input.source_code) === 'undefined' ) )
        throw [
            "CodeInfo Consructor needs object argument with `.last_inclusion.uri` or `.last_inclusion.depender` and `.source_code`;",
            "    `new CodeInfo({last_inclusion: {uri: '/bla.html'}})`",
            "/",
            "    `new CodeInfo({last_inclusion: {depender: new CodeInfo(/*...*/)}, source_code: new Buffer('/* src code */')})",
            "instead input ===",
            JSON.stringify(input)
        ].join('\n');

    assert( ! (input.last_inclusion||{}).depender || input.last_inclusion.depender.location.disk.directory || input.last_inclusion.depender.location.internet.url );
}

function set_root_depender(code){
    code.root_depender = code.root_depender || (code.last_inclusion.depender||{}).root_depender || code.last_inclusion.depender || null;
    assert( !! code.root_depender === !! code.last_inclusion.depender );
