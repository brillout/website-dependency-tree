...
function get_mutation_fcts(code__html, document, node_map){
    ...
    function remove_dependency(code__dependee, callback){

        apply_dom_changes();
        code__html.source_code = new Buffer(document.serialize());
        return code__html.retrieve_dependencies(callback);
        ...
        */
        ...
        function apply_dom_changes(){
            if( (code__dependee||{}).constructor !== CodeInfo ) {
                throw "argument to `"+remove_dependency.name+"` needs to be created with `new CodeInfo()`";
            }
    ...
    function insert_dependency(code__dependee, code__insertion_point, callback){
        ...
        function apply_dom_changes(){
            if( (code__dependee||{}).constructor !== CodeInfo ||
                ...
                throw "argument to `"+insert_dependency.name+"` needs to be created with `new CodeInfo()`";
            }
