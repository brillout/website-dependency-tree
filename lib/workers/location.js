...
function set_disk_location(code){
    if( ! code.location.uri ) {
        code.location.disk.directory = code.last_includer.location.disk.directory;
    }
    else if ( ! is_url( code.location.uri, code.last_includer ) ) {
        code.location.disk.path =
            code.location.disk.path
            || get_path(code.location.uri, code.last_includer) ;

        code.location.disk.directory =
            require('path').dirname(code.location.disk.path) ;

        code.location.disk.base =
            get_base(code.location.disk.directory, code.last_includer) ;
...
function set_internet_location(code){
    if( code.location.uri && is_url( code.location.uri, code.last_includer ) ) {
        code.location.internet.url = get_url(code.location.uri, code.last_includer) ;
        code.location.internet.path = parse_url(code.location.internet.url).path ;
        code.location.internet.is_cross_domain = code.last_includer && parse_url(code.last_includer.location.internet.url).host === parse_url(code.location.internet.url).host || null;
    }
}
...
function validate_location(code){
    ...
    validate(code.location, code.last_includer);
    return;
