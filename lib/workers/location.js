...
function set_disk_location(code){
    assert( !! code.root_includer === !! code.last_inclusion.includer ); // root_includer is required

    if( ! code.last_inclusion.uri ) {
        code.location.disk.directory = code.last_inclusion.includer.location.disk.directory;
    }
    else if ( ! is_url( code.last_inclusion.uri, code.last_inclusion.includer ) ) {
        code.location.disk.path =
            code.location.disk.path
            || get_path(code.last_inclusion.uri, code.last_inclusion.includer, code.root_includer) ;
...
function set_internet_location(code){
    if( code.last_inclusion.uri && is_url( code.last_inclusion.uri, code.last_inclusion.includer ) ) {
        code.location.internet.url = get_url(code.last_inclusion.uri, code.last_inclusion.includer) ;
        code.location.internet.path = parse_url(code.location.internet.url).path ;
        code.location.internet.is_cross_domain = code.last_inclusion.includer && parse_url(code.last_inclusion.includer.location.internet.url).host !== parse_url(code.location.internet.url).host || null;
    }
}

function validate_location(code){

    validate(code.location, code.last_inclusion.includer);
    return;
...
function get_path(uri, includer, root_includer){
