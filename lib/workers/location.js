...
function set_disk_location(code){
    assert( !! code.root_depender === !! code.last_inclusion.depender ); // root_depender is required

    if( ! code.last_inclusion.uri ) {
        code.location.disk.directory = code.last_inclusion.depender.location.disk.directory;
    }
    else if ( ! is_url( code.last_inclusion.uri, code.last_inclusion.depender ) ) {
        code.location.disk.path =
            code.location.disk.path
            || get_path(code.last_inclusion.uri, code.last_inclusion.depender, code.root_depender) ;
...
function set_internet_location(code){
    if( code.last_inclusion.uri && is_url( code.last_inclusion.uri, code.last_inclusion.depender ) ) {
        code.location.internet.url = get_url(code.last_inclusion.uri, code.last_inclusion.depender) ;
        code.location.internet.path = parse_url(code.location.internet.url).path ;
        code.location.internet.is_cross_domain = code.last_inclusion.depender && parse_url(code.last_inclusion.depender.location.internet.url).host !== parse_url(code.location.internet.url).host || null;
    }
}

function validate_location(code){

    validate(code.location, code.last_inclusion.depender);
    return;
