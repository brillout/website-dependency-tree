...
module.exports = function(code, done){
    ...
    if( code.type === 'jspm.js'  ) {
        handle_jspm(code, done);
    }
    else if( code.type === 'js' && code.source_code ) {
        handle_system_import(code, done);
    }
    else {
        done();
    }
...
function handle_jspm(code, done){
        ...
        get_jspm_dependencies(code.uri, code.base_path, function(info){
...
function handle_system_import(code, done){
    get_system_imports(code.id, code.source_code, function(dependencies_found){
...
function get_jspm_dependencies(uri, base_path, callback){
...
function get_system_imports(id, source_code, callback){
    var dependencies_found = [];
